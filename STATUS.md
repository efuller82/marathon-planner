...
- The full gate compiles the project and runs 237 unit tests using only
  synthetic data (239 once PR #20 merges); one Windows symbolic-link
  permission test skips.
...
- That error had to be retyped by hand, so issue #21 was filed and shipped
  the same day: a "Copy message" button in the status bar copies the full
  current message (errors and results alike) to the clipboard, with brief
  "Copied" feedback. Verified with DPI-aware screenshots at 150% scaling:
  the long MTP recovery message and the button fit the owner's display, and
  the clipboard received the complete text. One new headless test (gate
  237 green).
